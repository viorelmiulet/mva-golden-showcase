...
async function persistValidations(rows: { url: string; is_valid: boolean; status_code?: number; content_type?: string }[]) {
  ...
  const { error } = await sb.from('image_validation_cache').upsert(payload as any, { onConflict: 'url_hash' })
...
export async function handleFacebookCatalogFeed(req: Request): Promise<Response> {
