/**
 * Port of supabase/functions/immoflux-webhook/index.ts.
 * Receives property.created/updated/deleted/status_changed events from
...
function buildImmofluxSlug(
  p: ImmofluxWebhookPayload["data"],
  surface: number | null,
...
async function getWebhookSecret(supabase: any): Promise<string | null> {
  try {
    const { data } = await supabase
      ...
      .maybeSingle();
    return data?.value || process.env.IMMOFLUX_WEBHOOK_SECRET || null;
  } catch {
    return process.env.IMMOFLUX_WEBHOOK_SECRET || null;
  }
}

export async function handleImmofluxWebhook(req: Request): Promise<Response> {
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (isWithdrawn) {
      const { error: delErr } = await supabase.from("catalog_offers").delete().eq("external_id", externalId);
      if (delErr) {
