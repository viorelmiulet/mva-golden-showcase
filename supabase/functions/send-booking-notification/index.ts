...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const adminEmail = await sendMailgunEmail(
      ["contact@mvaimobiliare.ro"],
      `🏨 Rezervare nouă: ${propertyTitle}`,
      adminEmailHtml
    );
    ...
    if (guestEmail) {
      ...
      const guestEmailResponse = await sendMailgunEmail(
        [guestEmail],
        `✅ Confirmare cerere rezervare - ${propertyTitle}`,
        guestEmailHtml
      );
